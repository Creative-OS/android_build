...
""" % bcb_dev)
  ...
  date = target_info.GetBuildProp("ro.creative.build.date")
  version = target_info.GetBuildProp("ro.creative.version")

  if target_info.GetBuildProp("ro.product.model") is not None:
    model = target_info.GetBuildProp("ro.product.model")
    script.Print("***********************************************");
    script.Print("           CREATIVE-OS for %s"%(model));
    ...
    script.Print("***********************************************");
    script.Print("           CREATIVE-OS for %s"%(name));
    script.Print("   Version: %s"%(version));
    script.Print("   Compiled on: %s"%(date));
    script.Print("***********************************************");
